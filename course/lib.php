    ...
    if ($hasmanageactivities && !$mod->coursegroupmodeforce) {
        if (plugin_supports('mod', $mod->modname, FEATURE_GROUPS, 0)) {
            ...
            $actions[$actionname] = new action_menu_link_primary(
                ...
                \core\output\flex_icon::get_icon($groupimage, 'moodle', array('class' => 'iconsmall', 'alt' => $groupalt)),
                $grouptitle,
...
class course_request {
    ...
    public function approve() {
        ...
        $strmgr = get_string_manager();
        $this->notify($user, $USER, 'courserequestapproved', $strmgr->get_string('courseapprovedsubject'), get_string('courseapprovedemail2', 'moodle', $a), $course->id);
...
function course_get_user_administration_options($course, $context) {
    ...
    $options->restore = (has_capability('moodle/restore:restorefile', $context) or has_capability('moodle/backup:downloadfile', $context));
