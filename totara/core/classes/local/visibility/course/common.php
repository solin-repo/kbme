...
trait common {
    ...
    public function get_visible_counts_for_all_categories(int $userid): array {
        global $DB;
        [$where, $params] = $this->sql_where_visible($userid, 'c');
        $params['siteid'] = SITEID;
        $sql = "SELECT c.category, COUNT(c.id) as coursecount
                  FROM {course} c
                 WHERE {$where}
               GROUP BY c.category";
     ...
     */
    protected function get_availability_sql(int $userid, string $course_tablealias): ?array {
        return null;
    }
