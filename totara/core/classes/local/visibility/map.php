...
abstract class map {
     ...
     * @return string
     */
    final public function sql_view_hidden_roles(int $userid) {
        ...
        $sql = "SELECT map.{$field} AS id, COUNT(map.roleid) AS roles
                 FROM {{$table_map}} map
                 JOIN (
                 ...
                 ) vh_x ON vh_x.id = map.{$field} AND vh_x.roleid = map.roleid
                 GROUP BY map.{$field}";
