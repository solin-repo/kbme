 ...
 * @return bool
 */
function user_can_loginas(\stdClass $target_user, \stdClass $course = null): bool {
    global $USER, $SITE;

    if (empty($target_user->id) || $target_user->id <= 0 || !isset($target_user->deleted)) {
        throw new coding_exception('Login-as can only be checked for real users.');
    }
    ...
    if (is_siteadmin($target_user->id)) {
        // Logging in as an admin could lead to privilege escalation, it is strictly forbidden.
        return false;
    }
    if (isguestuser($target_user->id)) {
        // Login-as guest is not supported; if the site allows guest login it can be checked with an incognito window.
        return false;
    }
    ...
    if (!has_capability('moodle/user:loginas', $context_course)) {
        return false;
    }
    ...
    if ((!has_capability('moodle/course:view', $context_course) || !totara_course_is_viewable($course))
        && !is_enrolled($context_course, $USER->id, '', true)) {
        // Current user cannot enter the course, it means that the require_login() in course/loginas.php will likely fail.
        return false;
    }

    // We cannot do require_login() for other user, so let's just
    // check active enrolment which always grants course access.
    if (!is_enrolled($context_course, $target_user->id, '', true)) {
        ...
        // We cannot rely on user's course:view capability here due to the way login-as works.
        return false;
    }

    // Check if course has SEPARATEGROUPS and user is part of that group.
    if (groups_get_course_groupmode($course) == SEPARATEGROUPS &&
        !has_capability('moodle/site:accessallgroups', $context_course)) {
        $samegroup = false;
        if ($groups = groups_get_all_groups($course->id, $USER->id)) {
            foreach ($groups as $group) {
                if (groups_is_member($group->id, $target_user->id)) {
                    $samegroup = true;
                    break;
                }
            }
