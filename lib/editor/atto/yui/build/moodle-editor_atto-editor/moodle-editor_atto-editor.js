...
Y.extend(Editor, Y.Base, {
    ...
    initializer: function() {
        ...
        // Attach Editor instance to textarea.
...
EditorAutosave.prototype = {
    ...
    recoverText: function(text) {
        ...
        // Show a form-level alert for this Editor, as it has draft content. Other Editor
        // instances may also try to show this alert if there are more than one on the page,
        // but the notify module will limit this to only a single alert.
        this.showAlert(M.util.get_string('textrecoveredalert', 'editor_atto'), NOTIFY_WARNING);

        // To be very clear on usage of revert, we should also hide the recovery message if
        // the User does use the Undo button (if available). Also show a warning message about
        // recovered draft, visible until replaced.
        undo = this.toolbar.one('.atto_undo_button_undo');
        if (undo !== null) {
            langstring = 'textrecoveredwithundo';
            undo.once('click', this.hideMessage, this);
        } else {
            langstring = 'textrecovered';
        }
        this.showMessage(M.util.get_string(langstring, 'editor_atto'), NOTIFY_WARNING);

        // When the first content change is detected, the expectation of using Undo
        // to revert from the auto-applied Draft state to database stored state
        // must be reset - revert is no longer available as the user has now edited.
        this.once('change', this.hideMessage, this);
                ...
                on: {
                    ...
                    success: function(code, response) {
                        if (response.responseText !== "") {
                            ...
                            this.fire('atto:draftsavesuccess');
                        }
                    }
                }
            });
