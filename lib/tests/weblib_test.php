...
EXPECTED;
    ...
    public function test_validate_email_valid_addresses() {
        ...
        $this->assertEquals(1, validate_email('address@example.com'));
        $this->assertEquals(1, validate_email('firstname.lastname@example.com'));
        $this->assertEquals(1, validate_email('firstname+subject@example.com'));
        $this->assertEquals(1, validate_email('firstname.lastname+subject@example.com'));
        $this->assertEquals(1, validate_email('address@subdomain.example.com'));
        $this->assertEquals(1, validate_email('firstname+lastname@example.com'));
        $this->assertEquals(1, validate_email('address@123.123.123.123'));
        $this->assertEquals(1, validate_email('address@255.255.255.255'));
        $this->assertEquals(1, validate_email('1234567890@example.com'));
        $this->assertEquals(1, validate_email('123@example.123'));
        $this->assertEquals(1, validate_email('address@example-one.com'));
        $this->assertEquals(1, validate_email('_______@example.com'));
        $this->assertEquals(1, validate_email('_______@ex--ple.com'));
        $this->assertEquals(1, validate_email('address@example.name'));
        $this->assertEquals(1, validate_email('address@example.travel'));
        $this->assertEquals(1, validate_email('address@example.co.nz'));
        $this->assertEquals(1, validate_email('firstname-lastname@example.com'));
        $this->assertEquals(1, validate_email('address@io'));
        $this->assertEquals(1, validate_email('!#$%&amp`*+/=?^`{|}~@example.com'));
        $this->assertEquals(1, validate_email('address@io'));
        $this->assertEquals(1, validate_email('ADDRESS@EXAMPLE.COM'));
        $this->assertEquals(1, validate_email('Address@Example.Com'));
        $this->assertEquals(1, validate_email('Address.3@example.com'));
        $this->assertEquals(1, validate_email('user+mailbox/department=shipping@example.com'));
        $this->assertEquals(1, validate_email('abcdefghijklmnopqrstuvwxyz@example.com'));
        $this->assertEquals(1, validate_email('ABCDEFGHIJKLMNOPQRSTUVWXYZ@example.com'));
        $this->assertEquals(1, validate_email('firstname&lastname@example.com'));
        $this->assertEquals(1, validate_email('address@example'));
    }
    ...
    public function test_validate_email_valid_addresses_that_are_not_accepted() {
        ...
        $this->assertEquals(0, validate_email('address@[123.123.123.123]')); // Square bracket around IP address is considered valid, but not by us.
        $this->assertEquals(0, validate_email('address@[IPv6:2001:DB8::1]')); // Square bracket around IP address is considered valid, but not by us.
        $this->assertEquals(0, validate_email('address@::1')); // IPV6 addresses are valid, but not by us.
        $this->assertEquals(0, validate_email('address@2001:DB8::1')); // IPV6 addresses are valid, but not by us.
        $this->assertEquals(0, validate_email('address@IPv6:2001:DB8::1')); // IPV6 addresses are valid, but not by us.
        $this->assertEquals(0, validate_email('"address"@example.com')); // Quotes around email is considered valid, but not by us.
        $this->assertEquals(0, validate_email('“address”@example.com')); // Some utf8 quotes ;)
        $this->assertEquals(0, validate_email('""@example.com')); // This is valid as per RFC5321
        $this->assertEquals(0, validate_email('"\address"@example.com')); // This is valid as per RFC5321
        $this->assertEquals(0, validate_email('"\""@example.com')); // This is valid as per RFC5321
        $this->assertEquals(0, validate_email('"\\\\"@example.com')); // This is valid as per RFC5321
        $this->assertEquals(0, validate_email('"Firstname Lastname"@example.com')); // Traditionally valid email address.
        $this->assertEquals(0, validate_email('"Firstname.\\\\Lastname"@example.com')); // Traditionally valid email address.
        $this->assertEquals(0, validate_email('"very.unusual.@.unusual.com"@example.com'));
        $this->assertEquals(0, validate_email('"()<>[]:,;@\\\"!#$%&\'-/=?^_`{}| ~.a"@example.org'));
    }
    ...
    public function test_validate_email_valid_international_addresses_that_we_do_not_accept() {
        ...
        $this->assertEquals(0, validate_email('用户@例子.广告')); // Chinese.
        $this->assertEquals(0, validate_email('甲斐@黒川.日本')); // Japanese.
        $this->assertEquals(0, validate_email('संपर्क@डाटामेल.भारत')); // Hindi.
        $this->assertEquals(0, validate_email('юзер@екзампл.ком')); // Ukrainian.
        $this->assertEquals(0, validate_email('θσερ@εχαμπλε.ψομ')); // Greek.
        $this->assertEquals(0, validate_email('Dörte@Sörensen.example.com')); // German.
        $this->assertEquals(0, validate_email('аджай@экзампл.рус')); // Russian.
    }
    ...
    public function test_validate_email_invalid_addresses_that_are_accepted() {
        ...
        $this->assertEquals(1, validate_email('.address@example.com')); // Leading dot in address is not allowed, but we pass it.
        $this->assertEquals(1, validate_email('address.@example.com')); // Trailing dot in address is not allowed, but we pass it.
        $this->assertEquals(1, validate_email('address..address@example.com')); // Multiple dots, but we pass it.
        $this->assertEquals(1, validate_email('address@22.333.4444.55555')); // Invalid IP address, but a potentially valid domain.
        $this->assertEquals(1, validate_email('+address@example.com')); // Can't actually have an address starting with +
        $this->assertEquals(1, validate_email('.address@example.com')); // Can't actually have an address starting with .
        $this->assertEquals(1, validate_email('address+@example.com')); // Can't actually have an address ending with +
        $this->assertEquals(1, validate_email('address.@example.com')); // Can't actually have an address ending with .
        $this->assertEquals(1, validate_email('address@example.c-m')); // Invalid top level domain.
    }
    ...
    public function test_validate_email_invalid_addresses() {
        ...
        $this->assertEquals(0, validate_email('plainaddress'));
        $this->assertEquals(0, validate_email('@'));
        $this->assertEquals(0, validate_email('.@.'));
        $this->assertEquals(0, validate_email('address@'));
        $this->assertEquals(0, validate_email('@example.com'));
        $this->assertEquals(0, validate_email(' address@example.com'));
        $this->assertEquals(0, validate_email('address@example.com '));
        $this->assertEquals(0, validate_email('address@ example.com'));
        $this->assertEquals(0, validate_email('address @example.com'));
        $this->assertEquals(0, validate_email('address@example .com'));
        $this->assertEquals(0, validate_email('address@example. com'));
        $this->assertEquals(0, validate_email('address..example.com'));
        $this->assertEquals(0, validate_email('address.exa_mple.com'));
        $this->assertEquals(0, validate_email('firstname\@address@example.com')); // Can't escape the @ without quotes.
        $this->assertEquals(0, validate_email('#%^%#$@#$@#.com'));
        $this->assertEquals(0, validate_email('!#$%&amp;`*+/=?^`{|}~@example.com')); // This is just like an earlier test but with a ;
        $this->assertEquals(0, validate_email('@example.com'));
        $this->assertEquals(0, validate_email('Firstname Lastname <address@example.com>'));
        $this->assertEquals(0, validate_email('address.example.com'));
        $this->assertEquals(0, validate_email('address@example@example.com'));
        $this->assertEquals(0, validate_email('あいうえお@example.com'));
        $this->assertEquals(0, validate_email('address@example.com (Firstname Lastname)'));
        $this->assertEquals(0, validate_email('address@-example.com')); // Leading dash in front of domain is invalid
        $this->assertEquals(0, validate_email('address@example-.com')); // Training dash on domain is invalid
        $this->assertEquals(0, validate_email('address@.example.com')); // Leading dot in front of domain is invalid
        $this->assertEquals(0, validate_email('address@example..com')); // Training dot on domain is invalid
        $this->assertEquals(0, validate_email('"@example.com')); // Only a pair is valid.
        $this->assertEquals(0, validate_email('"""@example.com')); // Only a pair is valid.
        $this->assertEquals(0, validate_email('"\"@example.com'));
        // There were considered valid until Totara Learn 2.9.25.
        $this->assertEquals(0, validate_email('address@-example.com')); // Leading dash in front of domain is invalid
        $this->assertEquals(0, validate_email('address@example-.com')); // Training dash on domain is invalid
        $this->assertEquals(0, validate_email('address@example..com')); // Training dot on domain is invalid
        $this->assertEquals(0, validate_email('address@example.com.'));
        ...
        $this->assertEquals(0, validate_email('address@example.com='));
        $this->assertEquals(0, validate_email('address@example.com+'));
        $this->assertEquals(0, validate_email('address@example.com_'));
        $this->assertEquals(0, validate_email('address@example.com-'));
        $this->assertEquals(0, validate_email('address@example.com*'));
        $this->assertEquals(0, validate_email('address@example.com&'));
        $this->assertEquals(0, validate_email('address@example.com^'));
        ...
        $this->assertEquals(0, validate_email('address@example.com!'));
        $this->assertEquals(0, validate_email('address@example.com.'));
        $this->assertEquals(0, validate_email('address@exam#ple.com'));
        $this->assertEquals(0, validate_email('address@exam"ple.com'));
        $this->assertEquals(0, validate_email('address@exam?ple.com'));
        $this->assertEquals(0, validate_email('address@exam/ple.com'));
        $this->assertEquals(0, validate_email('address@exam~ple.com'));
        $this->assertEquals(0, validate_email('address@exam=ple.com'));
        $this->assertEquals(0, validate_email('address@exam+ple.com'));
        $this->assertEquals(0, validate_email('address@exam_ple.com'));
        $this->assertEquals(0, validate_email('address@exam*ple.com'));
        $this->assertEquals(0, validate_email('address@exam&ple.com'));
        $this->assertEquals(0, validate_email('address@exam^ple.com'));
        $this->assertEquals(0, validate_email('address@exam%ple.com'));
