 ...
 *             if 99 then default user's timezone is used {@link https://help.totaralearning.com/display/DEV/Dates+times+and+timezones#Datestimesandtimezones-Supportedtimezones}
...
function get_user_timezone($tz = 99) {
    ...
    while (((empty($tz) && !is_numeric($tz)) || $tz == 99) && $next = current($timezones)) {
        $tz = $next;
        // Also need to advance array cursor - can't use only call to next as it advances
        // the array cursor before the value is returned
        next($timezones);
    }
 ...
 *
 * TOTARA: Do not use this function.
 * It is fundamentally flawed, and the concept is a dead end.
 * If you are here because you are writing code then you need to change how you store your data.
 * Consider simply joining the data, or using JSON encoding if you need something a little more flexible.
 * Ideally look at what you are doing, perhaps there is a completely different, better, approach.
 *
 * @deprecated Don't use this function. Deprecated by Totara after it was added by Moodle.
 *
