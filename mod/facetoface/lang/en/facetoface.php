...
$string['allowschedulingconflicts_help'] = 'If trainers or users are already assigned or booked onto another face-to-face session at the same time as this session then the administrator will be warned, but can override such warnings and proceed anyway by selecting **Yes** from the dropdown menu.';
...
Available Face to Face placeholders:
...
$string['copy'] = 'Copy';
$string['cost'] = 'Cost';
$string['costinformation'] = 'Cost information';
$string['cancelbooking'] = 'Cancel booking';
...
$string['declareinterest_help'] = 'Displays an option within the face-to-face activity to allow a user to flag their interest and write a message without signing up.
...
$string['defaultsessiontimes'] = 'Default session times';
$string['defaultstarttime'] = 'Default start time';
$string['defaultstarttimehelp'] = 'Default start time for new sessions';
$string['defaultfinishtime'] = 'Default finish time';
$string['defaultfinishtimehelp'] = 'Default finish time for new sessions';
$string['defaultdaysbetweenstartfinish'] = 'Default days between start and finish';
$string['defaultdaysbetweenstartfinish_desc'] = 'The default number of days between the session start and finish.';
...
$string['defaultdaystosession'] = 'Number of days in future to default for new sessions';
$string['defaultdaystosession_desc'] = 'When creating a new session it\'s start and finish dates will default to this many days in the future.';
...
$string['deletereservationconfirm'] = 'Are you sure you want to delete all reservations in this session made by "{$a}"?';
...
$string['eventsessionsignup'] = 'Session signup';
$string['eventsignupstatusupdated'] = 'Signup status updated';
$string['eventbookingcancelled'] = 'Session booking cancelled';
$string['eventbookingrequestapproved'] = 'Session booking request approved';
$string['eventbookingrequestrejected'] = 'Session booking request rejected';
...
$string['f2f-waitlist-actions_help'] = 'The options are:

* **Confirm:** Book the selected users into the session and remove them from the wait-list.
* **Cancel:** Cancel the selected user\'s requests and remove them from the wait-list.
* **Play Lottery:** Fill the available places on the sessions with a random selection of the users from the wait-list. Users who are not selected will be left on the wait-list.';
...
$string['facetoface:managereservations'] = 'Manage reservations for a session';
...
$string['mincapacity_help'] = 'If the minimum capacity has not been reached by the cut-off point, then the appropriate users will be automatically notified. Users to be notified is determined by role assignments and the configuration setting under Site Administration > Plugins > Activity Modules > Face-to-face > General Settings > Notification recipients';
...
$string['noreservationsforsession'] = 'There are no reservations for this session.';
...
$string['notallowedtocancel'] = 'You are not allowed to cancel your booking in this session.';
$string['note'] = 'Note';
$string['notefull'] = 'Even if the Session is fully booked you can still register. You will be queued (marked in red). If someone signs out, the first learner in the queue will be moved into registeres learners and a notification will be sent to him/her by mail.';
...
$string['notifications_help'] = 'Here you can manage notifications for this face-to-face acitivity.';
...
$string['roomtype_help'] = 'The room type is used to determine if the system should try and prevent room booking conflicts. This will prevent a room from being booked for two sessions that are running at the same time.';
...
$string['selfapproval_help'] = 'This setting allows a user to confirm that they have sought approval to attend the session. Instead of their manager needing to approve their booking the user is presented with a checkbox when signing up and must confirm they have met the specified terms and conditions.
...
$string['sessiondefaults'] = 'Session Defaults';
...
$string['sessionfinishtime_help'] = 'When creating or editing a session, the session timezone may differ from the timezone for the Finish time. This is because the timezone for Finish time is determined by the timezone of the user creating or editing the session. The timezone of the user creating or editing the session does not affect the session timezone itself.';
...
$string['sessionstarttime_help'] = 'When creating or editing a session, the session timezone may differ from the timezone for the **Start time**. This is because the timezone for **Start time** is determined by the timezone of the user creating or editing the session. The timezone of the user creating or editing the session does not affect the session timezone itself.';
$string['sessiontimezone'] = 'Timezone';
$string['sessiontimezone_help'] = 'Select the timezone you want this session to be displayed in e.g. Pacific/Auckland. This will display the start time, finish time and timezone in accordance with the timezone selected. If you choose **User timezone**, this will display the start time, finish date, and timezone in relation to the timezone of the user viewing the session.';
...
$string['setting:sessionrolesnotify'] = 'The roles that will recieve the automated notifications for facetoface sessions, for example the minimum capacity warning';
...
$string['updateactivitieswarning'] = '<b>Warning:</b> If you choose to update all activities then all facetoface activities that have notifications based off this template will be updated with the changes that have been made.';
...
$string['upgradefixstatusapprovedlimbousersdescription'] = 'Learners with an invalid Face-to-Face signup status were detected.
The following learners were removed from sessions:<br>{$a}';
...
$string['allowbookingscancellationsdefault_help'] = 'Set the default cancellation settings for this face-to-face and allow users to cancel their bookings at any time, never or until cut-off reached (x amount of time before the session starts).';
...
$string['userwillbewaitlisted'] = 'This session is currently full. By clicking the "Join waitlist" button, you will be placed on the sessions\'s waitlist.';
...
$string['waitlisteveryone_help'] = 'Everyone who signs up for this session will be added to the waiting list.';
...
$string['details_help'] = 'Details are tracked on a per session basis.
...
Location displays on the course **Sign-up page**, the **View all sessions** page, and in all email notifications.

On the**View all sessions** page, the listed sessions can be filtered by location.';
$string['modulename_help'] = 'The face-to-face activity module enables a trainer to set up a booking system for one or many in-person/classroom based sessions.
...
An activity may be set to require manager approval and trainers can configure automated notifications and session reminders for attendees.
Learners can view and sign-up for sessions with their attendance tracked and recorded within the grades area.';
...
$string['recipients_help'] = 'Options are:

* **Booked:** Allows you to send the notification to all users who were booked, only those who attended, or only those who did not attend. Please note, when selecting **All booked**, notifications will be issued to all booked users, regardless of their attendance status, for sessions past and present.
* **Wait-listed:** Will send a notification to those who are signed up for a session which allows overbooking, but are not yet booked.
* **User cancelled:** Will send a notification to users for whom a session was cancelled or users who removed themselves from a session.
"facetoface","reminderinstrmngr_help","When **Send notice to manager** is checked, the text in the **Notice for Manager** field is sent to a learner\'s manager advising that they have signed up for a face-to-face session.';
...
$string['sessiondatetimeknown_help'] = 'If **Yes** is entered for this setting, the session date and time will be displayed on the course page (if the session is upcoming and available), the **View all sessions page**, the session sign-up page, as well as all email notifications related to the session.
...
If a session\'s date/time is not known (or wait-listed)and **No** is entered for this setting, the text **wait-listed** will be displayed on the course page, the **View all sessions page**, the session sign-up page, as well as all email notifications related to the session.
...
$string['showoncalendar_help'] = 'The options are:
...
* When **Course** is selected all of the face-to-face activity sessions will be displayed on the course calendar and as a **Course event** on the site level calendar and visible to all users enrolled in the course.
...
$string['suppressemail_help'] = 'Use this option if you want to silently add/remove users from a face-to-face session. When this option is toggled, the usual email confirmation is not sent to the selected users.';
...
When enteringmultiple email addresses, separate each address with a comma. For example: bob@example.com,joe@example.com';
