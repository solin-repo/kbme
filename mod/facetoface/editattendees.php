...
$attendees      = optional_param('attendees', false, PARAM_SEQUENCE);
...
// If an attendees list has not been sent (check here for
// the optional_param()'s default value) then prevent attendees
// being modified to mitigate against malformed URL params.
if ($attendees === false) {
    $clear = true;
}
...
if ($save && $onlycontent) {
    ...
    $_SESSION['f2f-bulk-results'][$session->id] = array($added, $errors, '', $suppressemail, $suppressccmanager);
...
if ($usercount <= MAX_USERS_PER_PAGE) {
    ...
    $extrauserfields = get_extra_user_fields_sql($context, 'u');
    $fields = $usernamefields . $extrauserfields;
    $availableusers = $DB->get_recordset_sql("SELECT u.id, $fields, u.email, ss.statuscode
                                        FROM {user} u
                                        LEFT JOIN {facetoface_signups} su
                                          ON u.id = su.userid
                                         AND su.sessionid = {$session->id}
