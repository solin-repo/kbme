...
function facetoface_cost($userid, $sessionid, $sessiondata) {
    ...
    $count = $DB->count_records_sql(
        "SELECT COUNT(*)
          ...
          WHERE su.sessionid = ?
            AND su.userid = ?
            AND (su.discountcode IS NOT NULL AND su.discountcode <> '')
            AND su.sessionid = se.id",
        array($sessionid, $userid)
    );

    if ($count > 0 && !$hidediscountconfig) {
        $cost = format_string($sessiondata->discountcost);
    } else {
        if (!$hidecostconfig) {
            $cost = format_string($sessiondata->normalcost);
        }
    }
    return $cost;
}
...
function facetoface_save_dates($session, array $dates = null) {
    ...
    $datesindb = array_values($olddates);
    ...
    // 0. Move out conflict dates.
    $uniquetime = 0;
    $get_unique_time = function () use (&$uniquetime, &$datesindb) {
        while (++$uniquetime) {
            foreach ($datesindb as $date) {
                if ((int)$date->timestart == $uniquetime || (int)$date->timefinish == $uniquetime) {
                    continue 2;
                }
            }
            return $uniquetime;
    ...
    if (!empty($dates)) {
        foreach ($dates as $date) {
            foreach ($datesindb as &$datedb) {
                if ((int)$date->id === (int)$datedb->id) {
                    continue;
                }
                $update = false;
                if ((int)$date->timestart === (int)$datedb->timestart) {
                    $datedb->timestart = $get_unique_time();
                    $update = true;
                }
                if ((int)$date->timefinish === (int)$datedb->timefinish) {
                    $datedb->timefinish = $get_unique_time();
                    $update = true;
                }
                if ($update) {
                    $DB->update_record('facetoface_sessions_dates', $datedb);
                }
            }
        }
    }
...
function facetoface_get_requests($sessionid) {
    ...
    $select = "u.id, su.id AS signupid, {$usernamefields}, u.email, u.deleted,
...
function facetoface_get_sessions_within($times, $userid = null, $extrawhere = '', $extraparams = array()) {
    ...
    if ($userid) {
        ...
        $params[]  = \mod_facetoface\signup\state\requested::get_code();
    }
