...
if (!$onlycontent && !$download) {
    if ($hasarchive) {
        echo $OUTPUT->notification(get_string('archivewarning_takeattendance', 'mod_facetoface'), \core\output\notification::NOTIFY_WARNING);
    }
    ...
    if (empty($rows)) {
        ...
        foreach ($rows as $attendee) {
            ...
            if ($attendee->id) {
                ...
                if (!$download) {
                    $attendeeurl = false;
                    if (is_enrolled($context, $attendee) && user_can_view_profile($attendee, $course)) {
                        $attendeeurl = new moodle_url('/user/view.php', array('id' => $attendee->id, 'course' => $course->id));
                    } else if (user_can_view_profile($attendee)) {
                        $attendeeurl = new moodle_url('/user/view.php', array('id' => $attendee->id));
                    }
                    if ($attendeeurl) {
            ...
            if ($actionallowed) {
                ...
                if (!$download) {
                    ...
                    $checkbox = '';
                    $disable = [];
                    if (!$attendee->archived) {
                        $checkbox_label =
                            get_string('takeattendance_tick', 'mod_facetoface', htmlspecialchars(fullname($attendee)));
                    ...
                    $select = html_writer::select($statusoptions, $optionid, $status, false, array_merge([ 'aria-label' => $select_label], $disable));
                    $data[] = $select;
                } else {
                    if (!$hidecost) {
