...
use \mod_facetoface\signup\state\{not_set, waitlisted, booked, requested, requestedadmin, user_cancelled, fully_attended, partially_attended};
...
class mod_facetoface_lib_testcase extends mod_facetoface_facetoface_testcase {
    ...
    public function test_rotate_session_dates() {
        ...
        $sessions = facetoface_get_session_dates($sessionid);
        $newdates = [
            (object)[
                'id' => $sessions[0]->id,
                'timestart' => $sessions[1]->timestart,
                'timefinish' => $sessions[1]->timefinish,
                'sessiontimezone' => '99',
                'roomid' => $rooms[0],
                'assetids' => []
            ],
            (object)[
                'id' => $sessions[1]->id,
                'timestart' => $sessions[2]->timestart,
                'timefinish' => $sessions[2]->timefinish,
                'sessiontimezone' => '99',
                'roomid' => $rooms[1],
                'assetids' => []
            ],
            (object)[
                'id' => $sessions[2]->id,
                'timestart' => $sessions[0]->timestart,
                'timefinish' => $sessions[0]->timefinish,
                'sessiontimezone' => '99',
                'roomid' => $rooms[2],
                'assetids' => []
            ]
        ...
        facetoface_save_dates($sessionid, $newdates);
        $transaction->allow_commit();

        $newsessions = facetoface_get_session_dates($sessionid);
        usort($newsessions, function ($x, $y) {
            return $x->roomid <=> $y->roomid;
        });
        $this->assertCount(3, $newsessions);
        $this->assertEquals($sessions[1]->timestart, $newsessions[0]->timestart);
        $this->assertEquals($sessions[2]->timestart, $newsessions[1]->timestart);
        $this->assertEquals($sessions[0]->timestart, $newsessions[2]->timestart);
    }
    ...
    public function test_shift_session_dates() {
        ...
        $this->assertCount(3, $newsessions);
        $this->assertEquals($times[1], $newsessions[0]->timestart);
        $this->assertEquals($times[2], $newsessions[1]->timestart);
        $this->assertEquals($times[3], $newsessions[2]->timestart);
        $this->assertEquals($rooms[0], $newsessions[0]->roomid);
        $this->assertEquals($rooms[1], $newsessions[1]->roomid);
        $this->assertEquals($rooms[2], $newsessions[2]->roomid);
    }
}
