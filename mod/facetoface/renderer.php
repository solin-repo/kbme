...
class mod_facetoface_renderer extends plugin_renderer_base {
    ...
    private function session_options_reserve_link($session, $signupcount, $reserveinfo = array()) {
        ...
        if ((!empty($session->registrationtimestart) && $session->registrationtimestart > $currentime)
            || (!empty($session->registrationtimefinish) && $currentime > $session->registrationtimefinish)) {
