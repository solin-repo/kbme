...
final class signup_helper {
    ...
    public static function get_failures(signup $signup) : array {
        ...
        $failures = $signup->get_failures(booked::class, waitlisted::class, requested::class);
        if (count($failures) > 1) {
            // Remove event_has_session because it's not very useful.
            unset($failures['event_has_session']);
        }
        return $failures;
    }
     ...
     * @param null          $grades         Do NOT pass this parameter in Totara 12
     * @return bool
     */
    public static function process_attendance(seminar_event $seminarevent, array $attendance, $grades = null) : bool {
        global $CFG, $USER;
        require_once($CFG->dirroot . '/mod/facetoface/lib.php'); // Necessary for facetoface_grade_item_update()

        if ($grades !== null) {
            throw new \coding_exception('Seminar attendance: Do not pass grades in Totara 12');
        }
        ...
        foreach ($attendance as $signupid => $value) {
            ...
            if (!$signup->can_switch($desiredstate)) {
                ...
                debugging("Seminar: could not switch signup id '$signupid' to '$desiredstate'");
                continue;
            }
