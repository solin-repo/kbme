...
function activitystatus_cm_info_view(cm_info $cm) {
    ...
    if ($completioninfo->is_tracked_user($USER->id) || $can_edit) {
        ...
        foreach ($iconsorder as $order) {
            ...
            if ($order->itemtype == 'mod' && $DB->record_exists('course_modules', ['id' => $order->courseormodid])) {
            ...
            } else if ($order->itemtype == 'course' && $DB->record_exists('course', ['id' => $order->courseormodid])) {
