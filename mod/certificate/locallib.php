...
function certificate_get_teachers($certificate, $user, $course, $cm) {
    ...
    $potteachers = get_users_by_capability($context, 'mod/certificate:receivenotification',
        '', '', '', '', '', '', false, false);
...
function certificate_get_images($type) {
    ...
    if (!empty($path)) {
        ...
        $options += certificate_retrieve_uploads($imagetype);
...
function certificate_retrieve_uploads($imagetype) {
    ...
    foreach ($files as $file) {
        ...
        $filename = $file->get_filename();
        $uploads[$filename] = substr($filename, 0, strlen($filename)-4);
    }
    return $uploads;
}
    ...
    if (!empty($path)) {
        switch ($certificate->$attr) {
            ...
            default :
                ...
                if ($filecontent = certificate_upload_exists($filetype, $name)) {
                    $pdf->Image($filecontent, $x, $y, $w, $h);
                }
                break;
        }
    }
...
function certificate_upload_exists($filearea, $filename) {
    $context = context_system::instance();
    $fs = get_file_storage();
